...
- IN PROGRESS [P0] [S] #18: Extend `scripts/copy-assets.mjs` to copy GLB companion textures — depends: — scope: `scripts/copy-assets.mjs`, `public/assets/`
...
- TODO [P0] [L] #7: Weapon entity — mesh attached to player hand transform, raycast firing with tracer line, muzzle-flash particle, ammo + reload — plus weapon database mapping the 22 mesh paths to archetypes — depends: #4 ✓, #5 ✓, #18 — scope: `src/entities/Weapon.ts`, `src/data/WeaponDatabase.ts`, `src/systems/Combat.ts`, `src/scenes/_dev/WeaponDemo.ts`, `src/entities/Player.ts`
...
- IN PROGRESS [P0] [L] #8: Enemy AI — zombie variants (Basic / Chubby / Ribcage) + UFO flyer with state machine `IDLE → CHASE → ATTACK → DEAD` — depends: #5 ✓ — scope: `src/entities/Enemy.ts`, `src/ai/EnemyStateMachine.ts`, `src/scenes/_dev/EnemyDemo.ts`
...
- IN PROGRESS [P0] [M] #9: Loot drop entity — rarity-colored vertical light beam, ground pickup mesh, interact-to-pickup — depends: #4 ✓, #5 ✓ — scope: `src/entities/LootDrop.ts`, `src/systems/LootSystem.ts`, `src/scenes/_dev/LootDemo.ts`
